...
use calyx_ir::{
    self as ir, Assignment, Control, FlatGuard, Group, Guard,
    GuardRef, RRC,
};
...
fn emit_component<F: io::Write>(
    ...
    for (idx, port_ref) in sig.ports.iter().enumerate() {
        ...
        let attr_str = format!(
            "(* {} *)",
            port.attributes.to_string_with(
                ", ",
                |k, v| format!("{}={}", k, v),
                |k, v| format!("{}={}", k, v.iter().join(", "))
            )
        );
        ...
        .filter_map(|cell| Some((cell, cell_instance(&cell.borrow()))))
    {
        ...
        let attr_str = format!(
            "(* {} *)",
            cell.borrow().attributes.to_string_with(
                ", ",
                |k, v| format!("{}={}", k, v),
                |k, v| format!("{}={}", k, v.iter().join(", "))
            )
...
fn emit_fsm_module<F: io::Write>(
    ...
    let mut unique_go_ports: BTreeMap<
        String,
    ...
    for assigns in fsm.borrow().merge_assignments().iter() {
        ...
        if is_go_port(dst) || is_done_port(dst) {
            ...
            unique_go_ports
                .insert(VerilogPortRef(dst).to_string(), assigns_at_time);
        }
    }
...
fn emit_fsm_transitions<F: io::Write>(
    trans: &ir::Transition,
    f: &mut F,
) -> io::Result<()> {
    match trans {
